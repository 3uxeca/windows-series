...
const FileUpload: React.FC<UploadDataProps> = ({ 
  ...
  const acceptValue = extList ? extList.map(ext => `${type}/${ext}`).join(", ") : '';

  const handleChangeFile = (e: React.ChangeEvent<HTMLInputElement> | React.DragEvent<HTMLDivElement>) => {
      ...
      {uploadData?.length === 0 ?
        <div className={`${styled.uploadWrap}`}>
          ...
          <div className={styled.uploadBox}>
            <div className={styled.dropZone} onDrop={e => onFileDrop(e)} onDragEnter={e => onDragEnter(e)} onDragOver={e => onDragOver(e)} onDragLeave={e => onDragLeave(e)}>
              <div className={styled.uploadTxt}>파일을 폴더에서 찾거나, 드래그 앤 드롭(Drag and Drop)으로 파일을 업로드 해주세요.</div>
                ...
                {uploadData?.length > 0 &&
                  uploadData.map((it, idx) => {
                    return (
                      ...
                      >
                        {it.name}
                        <div className={styled.delete} onClick={() => onRemove(idx)}>X</div>
                        {/* <img src={it} alt={it} /> */}
                      </div>
                    )
                  })
